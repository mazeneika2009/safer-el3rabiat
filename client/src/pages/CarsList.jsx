...
import {
  Filter, Phone, Search, UserCircle, Gauge, Calendar as CalIcon,
  MapPin, SlidersHorizontal, ChevronRight, LogOut
} from 'lucide-react';
...
const COLORS = ["All", "White", "Black", "Silver", "Gray", "Red", "Blue", "Brown", "Green", "Yellow", "Orange", "Gold", "Purple", "Bronze", "Burgundy"];
...
export default function CarsList() {
  ...
  useEffect(() => {
    fetchData('/cars')
      ...
      .catch(() => { setCars([]); setError('Inventory Synchronization offline.'); })
      .finally(() => setLoading(false));
  }, []);

  const handleMakeChange = (e) => { setSelectedMake(e.target.value); setSelectedModel(''); };

  const filteredCars = cars.filter(car => {
    const matchesMake = selectedMake ? car.make?.toLowerCase() === selectedMake.toLowerCase() : true;
      ...
      <header className="border-b border-zinc-800/60 bg-zinc-950/80 backdrop-blur-xl sticky top-0 z-30">
        <div className="max-w-screen-2xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            ...
            <button onClick={() => navigate('/admin')} className="hidden sm:flex items-center gap-2 text-xs font-bold text-zinc-500 hover:text-cyan-400 transition-colors uppercase tracking-[0.2em]">
              Control
            </button>
            {user ? (
              <>
                <button onClick={() => navigate(`/profile/${user.id}`)} className="flex items-center gap-2 text-sm font-medium text-zinc-400 hover:text-white transition-colors group">
                  <UserCircle className="w-6 h-6 text-zinc-500 group-hover:text-cyan-400 transition-colors" />
                </button>
                <button onClick={() => { localStorage.removeItem('user'); navigate('/login'); }} className="hidden md:flex items-center gap-2 text-[10px] font-bold text-zinc-600 hover:text-white transition-colors uppercase tracking-widest">
        ...
        <aside className="w-full lg:w-72 flex-shrink-0">
          <div className="bg-zinc-900/40 border border-zinc-800/80 p-6 rounded-2xl sticky top-24 shadow-2xl backdrop-blur-sm">
            <div className="flex items-center gap-2 mb-8">
              ...
              <h2 className="text-sm font-bold text-white uppercase tracking-wider">Refine Search</h2>
            </div>

            <div className="mb-6">
              <label className="block text-xs font-semibold text-zinc-500 mb-3 uppercase tracking-widest">Make</label>
              <select value={selectedMake} onChange={handleMakeChange} className="w-full bg-zinc-900 border border-zinc-800 text-zinc-300 py-3 px-4 rounded-xl focus:ring-1 focus:ring-cyan-500 focus:border-cyan-500 appearance-none text-sm font-medium transition-all">
                <option value="">All Makes</option>
                {Object.keys(CAR_DATA).map(make => <option key={make} value={make}>{make}</option>)}
              </select>
            </div>

            <div className={`mb-8 transition-all duration-500 ${selectedMake ? 'opacity-100' : 'opacity-20 pointer-events-none'}`}>
              <label className="block text-xs font-semibold text-zinc-500 mb-3 uppercase tracking-widest">Model</label>
              <select value={selectedModel} onChange={(e) => setSelectedModel(e.target.value)} disabled={!selectedMake} className="w-full bg-zinc-900 border border-zinc-800 text-zinc-300 py-3 px-4 rounded-xl focus:ring-1 focus:ring-cyan-500 focus:border-cyan-500 appearance-none text-sm font-medium transition-all">
                <option value="">Any Model</option>
                {selectedMake && CAR_DATA[selectedMake]?.map(model => <option key={model} value={model}>{model}</option>)}
              </select>
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-3 gap-2">
                {['All', 'New', 'Used'].map(c => (
                  <button key={c} onClick={() => setCondition(c)} className={`py-2.5 rounded-xl border text-sm font-bold tracking-wide transition-all ${condition === c ? c === 'New' ? 'bg-cyan-900/30 border-cyan-500/50 text-cyan-400' : 'bg-zinc-800 border-zinc-700 text-white' : 'bg-transparent border-zinc-800 text-zinc-500 hover:border-zinc-700 hover:text-zinc-300'}`}>
                    {c}
                  </button>
                ))}
              </div>
            ...
            <div className="mb-6">
              ...
              <div className="space-y-1 overflow-y-auto pr-1" style={{ maxHeight: '220px' }}>
                {COLORS.map(color => (
                  <button key={color} onClick={() => setActiveColor(color)} className={`w-full flex items-center justify-between px-4 py-2.5 rounded-xl transition-all border ${activeColor === color ? 'bg-zinc-800 border-zinc-700 text-white' : 'bg-transparent border-transparent text-zinc-400 hover:bg-zinc-900/50 hover:text-zinc-300'}`}>
                    <span className="text-sm font-medium">{color}</span>
                    {color !== 'All' && <span className="w-3 h-3 rounded-full border border-zinc-700 shadow-inner flex-shrink-0" style={{ backgroundColor: color.toLowerCase() }} />}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-zinc-800">
              <button onClick={() => { setSelectedMake(''); setSelectedModel(''); setActiveColor('All'); setSearchQuery(''); setCondition('All'); }} className="w-full py-3 text-xs uppercase tracking-widest font-semibold text-zinc-500 hover:text-zinc-300 transition-colors">
                Reset All Filters
              </button>
            </div>
          </div>
        ...
        <div className="flex-1">
          <div className="mb-6 flex items-end justify-between">
            ...
            <div className="md:hidden relative w-1/2 ml-4">
              ...
              <input type="text" placeholder="Search..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-3 py-2 bg-zinc-900 border border-zinc-800 rounded-lg text-sm text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-cyan-500/50" />
            </div>
          </div>
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="bg-zinc-900/40 rounded-2xl overflow-hidden border border-zinc-800 animate-pulse">
              ...
              {filteredCars.length > 0 ? filteredCars.map(car => (
                <div key={car.id} onClick={() => navigate(`/car/${car.id}`)} className="bg-zinc-900/40 rounded-2xl overflow-hidden border border-zinc-800 hover:border-zinc-600 transition-all duration-300 group flex flex-col hover:shadow-[0_0_30px_rgba(34,211,238,0.07)] hover:-translate-y-1 cursor-pointer">
                  <div className="relative h-52 overflow-hidden bg-zinc-900 border-b border-zinc-800">
                    <img src={car.image_url} alt={`${car.make} ${car.model}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 opacity-90 group-hover:opacity-100" />
                  ...
                  <div className="p-5 flex-1 flex flex-col relative">
                    ...
                    <div className="mb-4 pt-2">
                      <p className="text-xs text-cyan-500 font-semibold uppercase tracking-widest mb-1">{car.make}</p>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-6 text-xs text-zinc-400 font-medium">
                      <div className="flex items-center gap-1.5 bg-zinc-900 rounded-md p-2 border border-zinc-800/50">
                        <CalIcon className="w-3.5 h-3.5 text-zinc-500" />{car.year}
                      </div>
                      <div className="flex items-center gap-1.5 bg-zinc-900 rounded-md p-2 border border-zinc-800/50">
                        <MapPin className="w-3.5 h-3.5 text-zinc-500" />{(car.mileage || 0).toLocaleString()} mi
                      </div>
                      <div className="flex items-center gap-2 bg-zinc-900 rounded-md p-2 border border-zinc-800/50 col-span-2">
                        <div className="w-2.5 h-2.5 rounded-full border border-zinc-700 ml-1" style={{ backgroundColor: (car.color || 'white').toLowerCase() }} />
                        <span className="capitalize">{car.color || 'N/A'}</span>
                      </div>
                    </div>
                    <div className="mt-auto">
                      <a href={`tel:${car.phone}`} onClick={(e) => e.stopPropagation()} className="w-full py-3 px-4 bg-zinc-100 hover:bg-white text-zinc-900 font-bold tracking-wide rounded-xl text-center transition-colors flex items-center justify-between text-sm group/btn">
                ...
                <div className="col-span-full py-32 text-center flex flex-col items-center justify-center border border-zinc-800/50 rounded-2xl bg-zinc-900/20 border-dashed">
                  ...
                  <p className="text-zinc-500 mt-2 text-sm max-w-sm">We currently do not have any inventory that matches your criteria. Please try broadening your search.</p>
                  <button onClick={() => { setSelectedMake(''); setSelectedModel(''); setActiveColor('All'); setSearchQuery(''); setCondition('All'); }} className="mt-8 px-6 py-2 border border-zinc-700 bg-zinc-900 text-zinc-300 hover:text-white rounded-lg hover:border-zinc-500 transition-colors text-sm font-medium tracking-wide">
                    Clear All Filters
                  </button>
                </div>
              )}
