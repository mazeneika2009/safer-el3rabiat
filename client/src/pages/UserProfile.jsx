...
export default function UserProfile() {
  ...
  useEffect(() => {
    const getProfileData = async () => {
      ...
      try {
        ...
        setError('Identity Server unreachable.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleUpdateProfile = async (e) => {
    e.preventDefault();
    try {
      // In a real app, this would call the PUT /api/users/:id endpoint
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-300 font-sans tracking-wide selection:bg-cyan-500/30 selection:text-cyan-200">
        ...
        <div className="max-w-screen-xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-zinc-900 border border-zinc-800 shadow-lg cursor-pointer" onClick={() => navigate('/')}>
              <h1 className="text-xl font-bold text-white tracking-widest">S</h1>
            </div>
      ...
      <main className="max-w-screen-xl mx-auto px-6 py-12">
        ...
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 text-xs font-semibold tracking-widest text-zinc-500 hover:text-white uppercase transition-colors mb-10"
        >
                ...
                {isEditing ? (
                  <form onSubmit={handleUpdateProfile} className="space-y-4">
                    <input
                      ...
                      className="w-full bg-zinc-950 border border-zinc-800 rounded-xl p-3 text-sm text-white focus:ring-1 focus:ring-cyan-500"
                  ...
                  <>
                    ...
                    <div className="space-y-4 pt-6 border-t border-zinc-800/60">
                      ...
                      <div className="flex items-center gap-3 text-sm text-zinc-400">
                        <Calendar className="w-4 h-4 text-zinc-600" /> Joined {new Date(user?.created_at).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
                      </div>
                      <div className="flex items-center gap-3 text-sm text-zinc-400">
                    ...
                    <button
                      ...
                      className="w-full mt-8 flex items-center justify-center gap-2 py-4 bg-zinc-800 hover:bg-zinc-700 text-white text-xs font-bold tracking-widest uppercase rounded-2xl transition-all shadow-lg"
                    >
          ...
          <section className="lg:col-span-2 space-y-8">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="flex items-center gap-2 text-xs font-bold text-cyan-400 hover:text-white transition-colors uppercase tracking-widest"
              >
            ...
            {userCars.length > 0 ? (
                ...
                {userCars.map(car => (
                  <div key={car.id} className="bg-zinc-900/40 rounded-3xl border border-zinc-800 overflow-hidden hover:border-zinc-700 transition-all group cursor-pointer" onClick={() => navigate(`/car/${car.id}`)}>
                    <div className="h-44 relative bg-zinc-800 overflow-hidden">
                      <img src={car.image || car.image_url} alt={car.model} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                    ...
                    <div className="p-5">
                      <p className="text-[10px] font-bold text-cyan-500 uppercase tracking-widest mb-1">{car.make || car.brand}</p>
                      <h4 className="text-lg font-bold text-white mb-4">{car.model}</h4>
                      <div className="flex items-center justify-between text-[10px] font-bold text-zinc-500 uppercase tracking-widest">
                        <div className="flex items-center gap-1.5">
                          <MapPin className="w-3.5 h-3.5 text-zinc-700" /> Global Range
                        </div>
              ...
              <div className="py-24 text-center border-2 border-dashed border-zinc-800 rounded-3xl bg-zinc-900/20">
                ...
                <button
                  onClick={() => navigate('/seller')}
                  className="px-8 py-4 bg-white text-black text-xs font-bold tracking-widest uppercase rounded-2xl hover:scale-105 transition-transform"
                >
                  Post First Vehicle
                </button>
              </div>
            ...
            <div className="pt-12 border-t border-zinc-800/60 flex flex-col sm:flex-row items-center justify-between gap-6 opacity-40 grayscale group-hover:grayscale-0 transition-all">
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded bg-zinc-900 border border-zinc-800 flex items-center justify-center">
                  <CheckCircle className="w-4 h-4 text-zinc-400" />
                </div>
                <span className="text-[10px] font-bold uppercase tracking-widest">Verified Seller Account</span>
              </div>
              <div className="flex items-center gap-6">
                <div className="flex items-center gap-2">
                  <Phone className="w-4 h-4" />
                  <span className="text-[10px] font-bold uppercase tracking-widest">Support Core Ready</span>
                </div>
                <div className="flex items-center gap-2">
